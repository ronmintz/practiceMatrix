...
from past_behavior_v5A import past_behavior_delta
from past_behavior_v5A import past_behavior_alpha
from get_repo_quality4A import get_fraction_merged_for_users_and_repos
from get_repo_quality4A import get_fraction_of_issues_commented_for_users
from get_repo_quality4A import get_repo_quality
...
def do_something_per_agent(con: sqlite3.Connection, agent_id, num_repos,
    ...
    if PendNbrs == None:  # user not in group_roles table
        PendNbrs = '0'
    elif not str_is_number(PendNbrs):
        return None
    else:
    ...
    if pendant == None:   # user not in group_roles table
        pendant = '0'
    elif not str_is_number(pendant):
        return None
    ...
    while True:
        ...
        if PendNbrs == None:  # repo not in group_roles table
            PendNbrs = '0'
        elif not str_is_number(PendNbrs):  # garbage, try another row
            continue
        else:
